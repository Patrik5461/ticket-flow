...
- Každá `trigger_*()` funkcia má **na začiatku gating count** a pri prázdnej fronte sa vráti **bez** volania `net.http_post`. Gating podmienky: `email_jobs` / `refund_jobs` / `webhook_deliveries` = `pending` alebo (`failed` a `attempts < max_attempts`); `waitlist_entries` = `waiting` na type, kde `sold_count < capacity`; `settlements` = `invoice_attempts < 5` a zároveň buď (`invoice_status in ('none','failed')` a `fee_cents > 0`), alebo (`invoice_status = 'created'` a `invoice_ref is not null` a `invoice_sent_at is null`) — teda **obidva** priechody workera, vystavenie aj odoslanie (migrácia `20260805190000`).
