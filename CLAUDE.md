...
- **Neoverené ostáva samotné pg_cron plánovanie.** Ten test volal `trigger_email_processing()` cez PostgREST RPC, čiže scheduler obišiel — dokazuje, že funkcia funguje, keď ju niekto zavolá, nie že ju cron volá každých 5 minút. Rozdiel je podstatný: joby mohli byť `unschedule`-nuté alebo migrácia `cron.schedule` nemusela prejsť. Priamo sa to overí len `select * from cron.job` v dashboard SQL editore (cez REST schéma `cron` vidieť nie je), alebo nepriamo: nechaj vo fronte claimable riadok, **nevolaj RPC** a sleduj, či do 5 minút pribudne v nginx logu request s UA `pg_net`.
