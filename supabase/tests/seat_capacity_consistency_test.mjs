...
const soldCount = async () =>
  Number(
    (await one(`select sold_count from ticket_types where id=$1`, [ids.tt]))
      .sold_count,
  )
const statusCount = async (st) =>
  Number(
    (
      await one(
        `select count(*) c from event_seats where event_id=$1 and status=$2`,
        [ids.event, st],
      )
    ).c,
  )
...
async function setup(nSeats) {
  ids.org = (
    await one(
      `insert into organizers (name, slug) values ('ZZ Cap Org','zz-cap-'||substr(md5(random()::text),1,8)) returning id`,
    )
  ).id
  ids.venue = (
    await one(
      `insert into venues (organizer_id, name) values ($1,'ZZ Cap Venue') returning id`,
      [ids.org],
    )
  ).id
  ids.map = (
    await one(
      `insert into seat_maps (venue_id, name) values ($1,'ZZ Cap Map') returning id`,
      [ids.venue],
    )
  ).id
  ids.event = (
    await one(
      `insert into events (organizer_id, title, slug, starts_at, status) values ($1,'ZZ Cap Event','zz-cap-'||substr(md5(random()::text),1,8), now()+interval '30 days','published') returning id`,
      [ids.org],
    )
  ).id
  ids.tt = (
    await one(
      `insert into ticket_types (event_id, name, price_cents, capacity, seated) values ($1,'Seated',1000,$2,true) returning id`,
      [ids.event, nSeats],
    )
  ).id
  await db.query(
  ...
  for (let i = 0; i < nSeats; i++) {
    ...
    await db.query(
      `insert into event_seats (event_id, seat_id, ticket_type_id, status) values ($1,$2,$3,'available')`,
      [ids.event, seat, ids.tt],
    )
    ids.seats.push(seat)
  }
}
...
async function claimInTx(seatIds, orderId) {
  ...
  try {
    ...
    const r = await c.query(
      `select claim_seats($1, $2::uuid[], $3, 15) as ok`,
      [ids.event, seatIds, orderId],
    )
...
async function resetSeats() {
  await db.query(
    `update event_seats set status='available', held_until=null, order_id=null where event_id=$1`,
    [ids.event],
  )
...
async function run() {
  console.log(
    '=== Test 1: all-or-nothing block claim under contention (5 seats, 12 buyers) ===',
  )
  {
    const orders = await Promise.all(
      Array.from({ length: 12 }, () => newOrder()),
    )
    const results = await Promise.all(
      orders.map((o) => claimInTx(ids.seats, o)),
    )
    ...
    ok(
      'invariant sold_count == held+sold',
      (await soldCount()) ===
        (await statusCount('held')) + (await statusCount('sold')),
    )
    await resetSeats()
  }

  console.log(
    '=== Test 2: one-seat-per-buyer race (5 seats, 10 buyers, 2 per seat) → exactly 5 win ===',
  )
  {
    const attempts = []
    for (const seat of ids.seats) {
      for (let k = 0; k < 2; k++)
        attempts.push({ seat, order: await newOrder() })
    }
    const results = await Promise.all(
      attempts.map((a) => claimInTx([a.seat], a.order)),
    )
    const wins = results.filter(Boolean).length
    ok(
      'exactly 5 buyers won (no oversell, no under-fill)',
      wins === 5,
      `wins=${wins}`,
    )
    ...
    const dupes = Number(
      (
        await one(
          `select count(*) c from (select seat_id, count(distinct order_id) n from event_seats where event_id=$1 and status='held' group by seat_id having count(distinct order_id) > 1) x`,
          [ids.event],
        )
      ).c,
    )
  ...
  console.log('=== Test 4: capacity invariant after mixed ops ===')
  {
    ...
    await claimInTx([ids.seats[0], ids.seats[1]], o1)
    const o2 = await newOrder()
    await claimInTx([ids.seats[2]], o2)
...
async function cleanup() {
  try {
    ...
    await db.query(`delete from order_items where order_id = any($1::uuid[])`, [
      ids.orders,
    ])
