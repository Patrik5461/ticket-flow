...
const soldCount = async () =>
  Number(
    (await one(db, `select sold_count from ticket_types where id=$1`, [ids.tt]))
      .sold_count,
  )
const seatStatus = async (seatId) =>
  await one(
    db,
    `select status, order_id from event_seats where event_id=$1 and seat_id=$2`,
    [ids.event, seatId],
  )

async function run() {
  console.log(
    '=== Test 1: concurrent claim of the SAME seat — exactly one wins ===',
  )
  const a = mk(),
    b = mk()
  ...
  const aRes = (
    await one(a, `select claim_seats($1, array[$2]::uuid[], $3, 15) as ok`, [
      ids.event,
      ids.seatA,
      ids.orderA,
    ])
  ...
  const bPromise = one(
    b,
    `select claim_seats($1, array[$2]::uuid[], $3, 15) as ok`,
    [ids.event, ids.seatA, ids.orderB],
  )
  ...
  ok(
    'seatA held by exactly A',
    s1.status === 'held' && s1.order_id === ids.orderA,
    JSON.stringify(s1),
  )
  ok('sold_count = 1 after one claim', (await soldCount()) === 1)

  console.log(
    '=== Test 2: all-or-nothing — [held seatA, free seatB] → false, seatB untouched ===',
  )
  const c2 = (
    await one(
      db,
      `select claim_seats($1, array[$2,$3]::uuid[], $4, 15) as ok`,
      [ids.event, ids.seatA, ids.seatB, ids.orderC],
    )
  ).ok
  ok('mixed claim rejected', c2 === false, `got ${c2}`)
  ok(
    'seatB still available (undo worked)',
    (await seatStatus(ids.seatB)).status === 'available',
  )
  ok('sold_count unchanged = 1', (await soldCount()) === 1)

  console.log('=== Test 3: mark_seats_sold on payment ===')
  const sold = Number(
    (await one(db, `select mark_seats_sold($1) as n`, [ids.orderA])).n,
  )
  ...
  const rel = Number(
    (await one(db, `select release_seats_for_order($1) as n`, [ids.orderA])).n,
  )
  ok('1 seat released', rel === 1)
  ok(
    'seatA available again',
    (await seatStatus(ids.seatA)).status === 'available',
  )
  ok('sold_count back to 0', (await soldCount()) === 0)

  console.log(
    '=== Test 5: release_expired_orders frees held seats + no double-count for unseated ===',
  )
  // Hold seatA via orderExp, add an unseated order_item, expire the order, sweep.
  await q(db, `select claim_seats($1, array[$2]::uuid[], $3, 15)`, [
    ids.event,
    ids.seatA,
    ids.orderExp,
  ])
  await q(
    db,
    `insert into order_items (order_id, ticket_type_id, quantity, unit_price_cents) values ($1,$2,5,1000)`,
    [ids.orderExp, ids.ttUnseated],
  )
  await q(
    db,
    `update ticket_types set sold_count = sold_count + 5 where id=$1`,
    [ids.ttUnseated],
  ) // unseated reserve
  await q(
    db,
    `update orders set expires_at = now() - interval '1 min' where id=$1`,
    [ids.orderExp],
  )
  await q(db, `select release_expired_orders()`)
  ok(
    'expired order seat freed',
    (await seatStatus(ids.seatA)).status === 'available',
  )
  ok(
    'seated sold_count back to 0 (via seat release)',
    (await soldCount()) === 0,
  )
  const unseatedSold = Number(
    (
      await one(db, `select sold_count from ticket_types where id=$1`, [
        ids.ttUnseated,
      ])
    ).sold_count,
  )
  ok(
    'unseated sold_count back to 0 (via order_items, not doubled)',
    unseatedSold === 0,
    `got ${unseatedSold}`,
  )
  ok(
    'order marked expired',
    (await one(db, `select status from orders where id=$1`, [ids.orderExp]))
      .status === 'expired',
  )
}
...
async function cleanup() {
  ...
  try {
    ...
    await q(db, `delete from order_items where order_id = any($1::uuid[])`, [
      [ids.orderA, ids.orderB, ids.orderC, ids.orderExp].filter(Boolean),
    ])
